...
For each feed request, the ranking system:

1. **Retrieves candidate content** - All live content matching any filters
2. **Computes scores** - Applies signal weights to each item
3. **Applies editorial overrides** - Pins, boosts, and suppressions
4. **Returns ranked feed** - Ordered by final score

```mermaid
flowchart LR
    A[Content Pool] --> B[Filter]
    B --> C[Score]
    C --> D[Editorial Override]
    D --> E[Ranked Feed]
```
