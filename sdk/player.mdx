...
| `dismissFeed()` *(React Native only)* | Dismiss the currently presented feed expansion. Call it from an overlay via `ShortKitCommands.dismissFeed()`. No-op when the feed is embedded rather than presented modally. |
