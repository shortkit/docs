...
| `playbackIds` | `[String]?` | `nil` | Mux playback IDs to preload when using custom feed mode (`feedSource: .custom`). |
...
| `feedSource` | `FeedSource` | `.algorithmic` | `.algorithmic` fetches content from ShortKit's recommendation engine. `.custom` uses client-provided Mux playback IDs (pass via `playbackIds` on `ShortKit.init`). |
