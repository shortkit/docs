...
| `preload` | `FeedPreload?` | `nil` | iOS only. Seeds the feed with a client-curated opening tranche before algorithmic content. Build with `shortKit.preloadFeed(initialItems:)`. See [Preloading a custom tranche](/sdk/feed#preloading-a-custom-tranche-before-algorithmic-content). |
