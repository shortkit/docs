...
<Tabs>
  ...
  <Tab title="Web">
    ...
    // Filter by detected language — show only Spanish-language videos
    sk.createFeed(el, {
...
ShortKit automatically detects the spoken language in every video and stores it as `customMetadata.language` using a BCP 47 code. Use this with `metadata` to serve a language-specific feed without any extra work on upload.
