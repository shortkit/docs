...
<Tabs>
  ...
  <Tab title="Flutter">
    ...
    <Note>
      Flutter uses `overlay` (not `videoOverlay`) and sealed classes for enum-like types (`FeedHeight`, `OverlayConfig`).
    </Note>
  </Tab>
  <Tab title="React Native">
    ```typescript
    ...
    <Note>
      React Native uses different property names than the native SDKs: `overlay` (not `videoOverlay`), and does not support `adOverlay`. See [Overlays](/sdk/overlays) for details.
    </Note>
  </Tab>
  <Tab title="Web">
    ```js
...
| `carouselOverlay` | `CarouselOverlayMode` | `.none` | `.none` disables carousel UI. `.custom { ... }` provides a developer-built carousel overlay. See [Overlays](/sdk/overlays#carousel-and-ad-overlay-modes). |
...
| `adOverlay` | `AdOverlayMode` | `.none` | `.none` disables native ad overlays. `.custom { ... }` provides a developer-built ad overlay. See [Overlays](/sdk/overlays#carousel-and-ad-overlay-modes). Only applies to native ads; system-defined ads (IMA/VAST) are unaffected. |
