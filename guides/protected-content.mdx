...
<Note>
  Protected playback works the same in the [single-player component](/sdk/player-and-widget#single-player) on iOS and Android. The player resolves the signed streaming URL on activation and loads the signed thumbnail for the inactive-card poster, so a protected card shows its real first frame instead of a blank placeholder.
</Note>
