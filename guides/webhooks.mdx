...
Webhooks notify your server when important events occur in shortkit, such as content finishing processing or live streams starting.
...
| Event | Trigger |
|-------|---------|
| `live-stream.started` | Live stream went live |
...
curl -X PATCH https://api.shortkit.dev/v1/webhooks/whk_abc123 \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "events": ["content.ready", "content.errored", "live-stream.started"]
  }'
```
...
<Columns cols={2}>
  <Card title="Content API" icon="code" href="/api-reference/endpoints/content">
    Learn about content lifecycle events.
  </Card>
  <Card title="Live streaming" icon="video" href="/infrastructure/live-streaming">
    Set up live stream webhooks.
  </Card>
</Columns>
