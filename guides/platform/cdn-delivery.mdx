...
Content flows through three tiers: users request from edge nodes, which pull from the origin shield, which pulls from origin storage. Cache hits at any tier return content immediately without hitting the next tier.
