...
When a user taps the share button, the SDK generates a shareable URL. When the recipient taps this link, the system checks if your app is installed. If so, the app opens directly to that video. If not, they're taken to a web fallback (your website or app store).
