...
| `/v1/content/upload` | POST | Upload video files (requires `sdk_uploads_enabled`) |
| `/v1/content/{id}/upload-url` | POST | Re-mint or cancel an upload URL (requires `sdk_uploads_enabled`) |
| `/v1/content/{id}/status` | GET | Poll upload lifecycle (requires `sdk_uploads_enabled`) |
