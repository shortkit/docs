...
<ResponseField name="customMetadata" type="object">
  ...
  - **`language`** — BCP 47 language code detected from the video's audio track (e.g. `"en"`, `"es"`, `"ja"`). Set asynchronously after processing completes. `"und"` indicates no speech was detected. You can filter the feed by this key using [`FeedFilter.metadata`](/sdk/configuration#feedfilter).
</ResponseField>
