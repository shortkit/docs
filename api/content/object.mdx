...
<ResponseField name="isDownloadable" type="boolean">
  `true` when the item's organization has static renditions enabled and the item is eligible for [download](/sdk/downloads). Use this to gate download affordances in your UI.
</ResponseField>

<ResponseField name="downloadUrl" type="string | null">
  Direct URL to the static MP4 rendition, when available. `null` while the rendition is still preparing — the SDK refreshes this lazily when you call `downloadVideo`. Short-lived; do not persist across sessions.
</ResponseField>
