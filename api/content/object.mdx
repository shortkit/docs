...
<ResponseField name="streamingUrl" type="string | null">
  HLS streaming URL for video playback. Available once `uploadStatus` is `ready`. `null` for carousels. Also `null` for [protected content](/guides/protected-content) — the streaming URL is withheld until a signed URL is issued via the [playback-token endpoint](/api/content/playback-token).
</ResponseField>

<ResponseField name="protected" type="boolean">
  When `true`, playback is gated behind a short-lived signed streaming URL and `streamingUrl` is withheld (`null`). Issue a signed URL via the [playback-token endpoint](/api/content/playback-token), or let the SDK handle it automatically. See [Protected Content](/guides/protected-content).
</ResponseField>
