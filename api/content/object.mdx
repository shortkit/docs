...
<ResponseField name="uploadStatus" type="string">
  Tracks media processing. One of `queued`, `processing`, `ready`, or `errored`. See [Upload lifecycle](#upload-lifecycle) below.
</ResponseField>

<ResponseField name="errorReason" type="string | null">
  Human-readable explanation of why processing failed. Set when `uploadStatus` is `errored` — for example, when an imported source is an unsupported content type. `null` in every other state.
</ResponseField>
...
|---------------|-------------|
| `queued` | Upload or import initiated, media not yet received or not yet processing |
...
| `errored` | Processing failed — `errorReason` carries the reason |

**Videos** move through the full lifecycle: `queued` → `processing` → `ready`.
