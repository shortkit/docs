...
Deleting a content item via `DELETE /v1/content/{id}` permanently removes it. The item is immediately dropped from the feed and no longer appears in list, search, or count results. The underlying media assets are cleaned up asynchronously. Deletion is not reversible.
