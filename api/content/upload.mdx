...
Sent when your file has been received and processing has begun. This is where you receive the `playbackId` — store it alongside your content record.
...
The video is streamable at this point, but transcoding is not yet complete. Viewers of longer videos may experience buffering near the end until encoding finishes.
...
Sent if processing fails. The payload includes an `error` field describing the failure. No `playbackId` is included.
...
A single callback fires once all images have been validated and the carousel status is determined:
